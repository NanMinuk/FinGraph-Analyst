...
LLM 기반 intent classification으로 질의를 다음 유형으로 분류합니다.
...
금융 뉴스 문서를 chunk 단위로 분할하고, Chroma 기반 semantic retrieval을 수행합니다.

### 3. LLM-based relation extraction
뉴스 본문에서 기업과 이벤트 간 관계를 추출합니다.
...
모든 관계를 저장하지 않고, post-processing 이후 남은 **고신뢰 relation만 Neo4j에 selective upsert** 합니다.

### 5. Hybrid Graph Context
현재 질의에서 추출한 relation과 기존 그래프에 축적된 persistent relation을 결합해  
`current / persistent / hybrid` source type을 구분합니다.

### 6. Explainable report generation
최종 결과를 단순 답변이 아니라 다음 요소와 함께 제공합니다.
...
### 7. Streamlit UI
분석 결과를 보고서와 그래프 시각화 형태로 확인할 수 있습니다.
